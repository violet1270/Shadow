    ...
    <div class="main-wrap">
      ...
      <div class="search-wrap">
        <div class="search-content">
          <form action="admin_bagmgr.php" method="post">
              ...
              <tr>
                ...
                <td>
                  <select name="search-type" id="">
                    ...
                    <option value="designer">生产厂商</option>
                    <option value="type">类型</option>
                    <option value="color">颜色</option>
                    <option value="status">租赁状态</option>
                  </select>
                </td>
                <th width="70">关键字</th>
        ...
        <div class="result-content">
          <table class="result-tab" width="100%">
            <tr>
              ...
              <th class="tc">上市日期</th>
              <th class="tc">租赁状态</th>
              <th class="tc">租赁价格（元/天）</th>
              <th class="tc">操&emsp;&emsp;作</th>
            </tr>
            <?php
              require("../dbconnect.php");
              $pagesize=20;
              $sql = "select * from handbags where ".@$_POST["search-type"]." like ('%".@$_POST["keywords"]."%')";
              //echo $sql;
              ...
              $sql="select * from handbags where ".@$_POST["search-type"]." like ('%".@$_POST["keywords"]."%') order by bagid asc limit $startno,$pagesize";
              //echo $sql;
              $rs=mysqli_query($db_link,$sql);
              if(!$rs)
              {
              ...
              while($rows=mysqli_fetch_assoc($rs))
              {
                ...
                echo "<th class='tc'>".$rows["status"]."</th>";
                echo "<th class='tc'>".$rows["price"]."</th>";
                echo "<th class='tc'>";
                echo "<a href='admin_bagmod.php?mid=".$rows["bagid"]."'  class='link-update'>修改</a>&nbsp;&nbsp;<a href='delete.php?mid=".$rows["bagid"]."' class='link-del''>删除</a>";
                echo "</th>";
                echo "</tr>";
              }
            ?>
